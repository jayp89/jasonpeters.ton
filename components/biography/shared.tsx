...
export const SubHeader: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <h3 className="font-orbitron text-2xl font-bold text-amber-200 mt-12 mb-6 pl-4 border-l-4 border-amber-500/50">{children}</h3>
);

export const SubSubHeader: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <h4 className="text-xl font-bold text-white mt-10 mb-4 flex items-center">
...
export const SubSubSubHeader: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <h5 className="text-lg font-bold text-amber-400/90 mt-8 mb-3 italic">{children}</h5>
);

export const BioParagraph: React.FC<{ children?: React.ReactNode; className?: string; html?: string }> = ({ children, className, html }) => {
    // Added text-justify and increased bottom margin for better readability
    const baseClasses = "text-gray-300 leading-loose text-lg mb-8 tracking-wide text-justify";
    if (html) {
...
export const BioTable: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <div className="overflow-x-auto my-10 rounded-lg border border-white/10 shadow-lg bg-black/20">
...
export const BioTableHeader: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <th className="p-4 md:p-5 font-bold text-amber-300 border-b border-white/10 font-orbitron tracking-wider text-sm uppercase">{children}</th>
);
...
export const BioTableCell: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <td className="p-4 md:p-5 align-top text-gray-300 text-base leading-relaxed group-hover:text-white transition-colors">{children}</td>
);

export const BioOrderedList: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <ol className="list-decimal list-outside ml-6 space-y-4 text-gray-300 leading-relaxed text-lg mb-10 marker:text-amber-500 marker:font-bold">{children}</ol>
);

export const BioListItem: React.FC<{ children?: React.ReactNode; html?: string }> = ({ children, html }) => {
    if (html) {
        return <li className="pl-3" dangerouslySetInnerHTML={{ __html: html }} />;
    }
